...
数学上のモデルをそのまま計算機上に作り上げるのには限界があります。幅や高さのわずかな変化は、今回は目をつぶることにしましょう。一方、もともと一致していた点がズレてくるのは少し問題なので、案2を採用することにします。移動するときに座標値を書き換えるのではなく、座標変換行列を使うようにすれば、幅、高さの変化をある程度回避できます。
...
AIに作ってもらいました。AIの案は、CVec2d.x, yでしたがvx, vyに修正しました。
...
対角の2点をからBox2dを作ります。コンストラクタに与える2点の順序に決まりはありません。
ですが内部では対角の2点をmin, maxとして持ちます。座標の小さい方をmin, 大きい方をmaxに設定します。
矩形の角は4個ありますが、どれを原点として覚えるかに意味は持たせたくないので4点のうち座標値の小さいものを原点(min)と決めます。
getMin() と getMax() では内部のVec2dをそのまま返しています。セッターをどうするかはまだ決めていません。

## 描画クラスを作る

canvasに描画しますが、ここは一つ間に描画クラスgraph2dを挟むことにします。
こうしておけばズーム、パンなどのカメラ操作もここで全部吸収できるので便利です。
...
AIに相談してindex.htmlにtypeを付け加えました。
